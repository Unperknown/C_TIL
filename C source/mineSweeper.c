...
int main()
{
	...
	for (int i = 0; i < column; i++)
		mneS[i] = malloc(sizeof(char) * row); //mneS 포인터에 가로와 세로 크기 만큼의 동적 메모리 할당을 통해 2차원 배열처럼 사용
	...
	for (int i = 0; i < column; i++)
	{
		for (int j = 0; j < row; j++) // 각 열의 행 위치를 차례대로 따라가서
		{
			if (mneS[i][j] == '.')
				mneS[i][j] = '0';
			else if (mneS[i][j] == '*') //그 위치가 지뢰이면
			{
				for (int k = i - 1; k <= i + 1; k++)
				{
					...
					{
						if (k >= 0 && k < column && l >= 0 && l < row) //(단, 그 위치는 배열 인덱스 값에서 벗어나면 안됨)
						{
							if (mneS[k][l] == '.' || mneS[k][l] == '0')
								mneS[k][l] = '1'; //한 칸 떨어진 위치가 '.'('0')이면 지뢰가 있음을 보여주기 위해 '1' 할당
							else if (mneS[k][l] >= '1')
